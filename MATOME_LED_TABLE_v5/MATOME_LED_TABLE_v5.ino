...
//-------------------------------
    ...
    if(old) digitalWrite(buzer,HIGH);
    else digitalWrite(buzer,LOW);//fix to high or low
...
void loop() {
   wdt_reset();
   //-----------main code -----------
   if(debug){ send_get("AT+CWLAP","OK",1000);}
   ...
   if(DATA==""){// yeu cau gui data
    ...
    case '5'://disconnect AP
         ...
         lcd.setCursor(0,3);lcd.print("CONNECTING...");  
         send_get("AT+CWJAP_DEF=\"matome_yx0_l2\",\"25021993\"","OK",5000);
         return;
...
{   String response = "";
    ...
    //SAVE DATA1
     if((response.indexOf("MODEL"))>-1&&(response.indexOf("STEP")>-1)&&(response.indexOf("TARGET")>-1)){
       DATA="";
       int first_ = response.indexOf(":");
       int second_ = response.indexOf("\r\n",first_+1);
